...
public class pre_Account implements imp_Account {
        ...
        mRefAccount.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.getValue() != null) {
                    tb_Account.getInstance(context).addAccount(new objAccount(dataSnapshot.getKey(), dataSnapshot.getValue(Users.class)));
    ...
    private void writeAccountToDatabase(final FirebaseUser user, String type) {
        ...
        final Users mAccount = new Users(user.getEmail(), type);
     ...
     */
    @Override
    public void profileSetting(Uri uriAvatar, Users mAccount) {
        if (checkInputSettingProfile(mAccount)) {



            //Update avatar
            if (uriAvatar.getUserInfo()!= null) {
                updateAvatar(uriAvatar,mAccount);
            } else {
                mAccount.setImage(keyUtils.NULL);
...
//    }
     ...
     */
    public void updateAvatar(Uri uriAvatar, final onResultUpdateAvatar mAccount) {
        try {
                    ...
                    storageReference.getDownloadUrl()
                            .addOnSuccessListener(new OnSuccessListener<Uri>() {
                                ...
                                public void onSuccess(Uri uri) {
                                    //Write to database
                                    mAccount.setImage(uri.toString());
                                    updateProfileToDatabase((Users) mAccount);
                                }
                            })
                            .addOnFailureListener(new OnFailureListener() {
                                @Override
     ...
     * @param mAccount object account
     */
    public void updateProfileToDatabase(final Users mAccount) {
        DatabaseReference mRefAccount = mDatabase.getReference().child(keyUtils.accountList).child(mUser.getUid());
    ...
    public void updateAvatar(Uri uriimage, @NonNull Users result) {
        try {
            StorageReference avatarRef = mStorage
                    .getReference()
                    .child(keyUtils.REF_STORAGE_AVATAR)
     ...
     */
    private boolean checkInputSettingProfile(Users mAccount) {
